...
name: autopilot
description: >-
  Intake-to-delivery pipeline. Processes pending items from .citadel/intake/:
  briefs new ideas, executes approved work through research → plan → build → verify.
  Drop a file in .citadel/intake/ and invoke this skill.
user-invocable: true
auto-trigger: false
...
Use Autopilot when:
- There are pending items in `.citadel/intake/`
...
- Large, multi-session campaigns (use the archon skill)
- Parallel execution (use the fleet skill)
- Exploratory or open-ended work (use the marshal skill)
...
2. Execute the approach:
   ...
   - Follow the project's conventions (CLAUDE.md or equivalent project guide)
...
- Never build without reading the project guide first
