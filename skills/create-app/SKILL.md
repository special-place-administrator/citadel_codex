...
working application. It integrates the prd skill, architect skill, and archon skill
into a single flow with human checkpoints at the right moments.
...
- Action: Run the scaffold skill with stack detection. No PRD, no architecture.
...
- Action: Run prd skill (asks clarifying questions) → user approves → architect skill → user approves → archon skill
...
- Action: Load template PRD if available → architect skill with template defaults → archon skill
- Use when: The app shape is known and common. Speeds up the planning phase.
- Template detection: check `.citadel/_templates/app-types/` for matching templates.
...
- Action: prd skill (minimal questions) → architect skill (auto-approve if confidence high) → archon skill with self-correction loop
...
- Action: prd skill in feature mode → architect skill in existing codebase mode → archon skill
...
**Tier 1:** Invoke the scaffold skill. Done.

**Tier 2:**
1. Invoke the prd skill with the user's description
2. Wait for user approval of the PRD
3. Invoke the architect skill with the approved PRD
...
**Tier 3:**
1. Check `.citadel/_templates/app-types/` for a matching template
2. If found: present the template PRD, ask "Does this match? What would you change?"
3. User approves or modifies → architect skill with template defaults
4. Brief architecture review (faster than Tier 2 since the shape is known)
5. Create and execute Archon campaign

**Tier 4:**
1. Invoke the prd skill in express mode (ask at most 1 question, or 0 if the description is complete)
2. User approves PRD (this is the only mandatory human checkpoint)
3. Invoke the architect skill (auto-approve if all end conditions are machine-verifiable)
...
2. Invoke the prd skill in feature mode (reads codebase before asking questions, max 2 questions)
3. User approves feature spec (one mandatory checkpoint)
4. Invoke the architect skill in existing codebase mode:
...
2. Check each one:
   - Run commands, check file existence, invoke the live-preview skill for visual checks
3. Report:
...
- Suggest the postmortem skill to generate a campaign postmortem
...
- **No stack lock-in**: the architect skill chooses the stack based on the PRD,
  not a hardcoded default. Different apps get different stacks.
