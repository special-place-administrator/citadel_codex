...
The design skill creates a design manifest that documents the visual language of a project.
...
- When the router routes "design", "style guide", "visual consistency", "design manifest"
...
Write to `.citadel/design-manifest.md`:
...
The post-edit hook (post-edit.js) checks for `.citadel/design-manifest.md`.
If it exists:
...
- Accumulate patterns automatically (that's a future capability — for now, manifests are explicit)
