...
description: >-
  ...
  before they compound. Playwright is an optional dependency (graceful skip if
  not installed).
user-invocable: true
auto-trigger: false
...
- When orchestrating agents delegate UI work
...
1. Screenshots go to `.citadel/screenshots/{campaign-slug}/` (if in a campaign)
   or `.citadel/screenshots/` (if standalone)
...
1. After the sub-agent completes, the orchestrator invokes live-preview on the modified routes
2. If any route is BLANK or FAIL, the phase is NOT marked complete
3. The fix cycle runs before proceeding to the next phase
4. This is part of the orchestrator's self-correction quality spot-check
